...
void* clientFunction(void *arguments){
	...
	while(1){
		...
		if(strcmp(op_buff, "SUBSCRIBE") == 0){
			...
			getTopicAndText("localhost","karate", retText );
...
/* function initialazing the storage*/
void initializeStorage(char* host)
{
	CLIENT *clnt;
	...
	clnt = clnt_create (host, STORAGE, STORAGEVER, "udp"); // Client handle is created
	if (clnt == NULL) {
...
/* function storing Topic and Text to storage*/
int putTopicAndText(char* host, char* topic, char* text)
{
...
/* function getting Text by Topic from storage */
int getTopicAndText(char* host, char* topic, char* text)
{
	CLIENT *clnt;
	...
	if (retval_3 != RPC_SUCCESS) {
	 ...
	 //printf("GET TEXT BY TOPIC IS SUCCESS %s\n", result_3);
	// while ( *result_3 ) printf("RECEIVED TEXT: %s\n", *result_3++ );
